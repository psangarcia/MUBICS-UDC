...
% Número de vecinos (nearest miss y nearest hit) para relieff
% Parámetros configurables
numNeighbors=10;
nFeat=20; % Se establece en torno a la mitad de las totales

% En lo que respecta a la versión de la red, se establecen parámetros:
...
filename = 'qsar_lm_84_relief'; 

selectedfeatures=zeros(iterac,k,nFeat);
for iter=1:iterac
    CV = cvpartition(outputs,TypeCV,k);
    for i=1:k
        trIdx=CV.training(i);

        [ranked,weight]=relieff(inputs(trIdx,:),outputs(trIdx),numNeighbors);
  
        %SE COMPRUEBA QUE NO SELECCIONAMOS FEATURES CON PESOS NEGATIVOS:
        weight(ranked([1:nFeat]));

        %Las variables seleccionadas por cada iteración y fold
        %están a 1, el resto a 0
        selectedfeatures(iter,i,ranked([1:nFeat]))=1;
     
        %Todas las entradas seleccionando solo las nfeat de mayor peso:
        inputs_relief=inputs(:,ranked([1:nFeat]));
        
        % Entrenamiento de la versión de red planteada:
        ...
        %Se procede al entrenamiento de las versiones de las redes
...
% Visualización de características empleadas en la mitad (o más) de las ejecuciones:
meanFeat=[];
for s=1:nFeat
    if ((sum(sum(selectedfeatures(:,:,s))) / iterac*k )>0.5)
        meanFeat=[meanFeat s]
    end
end

% Visualización de características seleccionadas en todas las ejecuciones:
allFeat=[];
for s=1:nFeat
    if ((sum(sum(selectedfeatures(:,:,s))) / (iterac*k) )==1)
        allFeat=[allFeat s]
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%% COMPARACIÓN DE RELIEF CON TÉCNICAS ANTERIORES  %%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear all; close all;
...
load('qsar_lm_84_relief.mat');
Aux_Relief=mean(AuxF1Score_TEST,2);
Relief = Aux_Relief([1:3:15]);

Muestras = [DiscrimLineal, DiscrimQuadr, Tree, SVMLineal, RNA, Relief];
etiqueta = ['Disc_lineal';'Disc_cuadra'; 'Arbo_decisi'; 'Svmi_lineal'; 'redn_lineal'; 'qsar_Relief'];
